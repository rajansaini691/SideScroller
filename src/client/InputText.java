...
public class InputText implements KeyListener {
	...
	/**
	 * Stores reference to Comp to repaint the screen with each key press
	 */
	private Comp comp;
	 ...
	 * @param height
	 */
	public InputText(int x, int y, int width, int height, Comp comp) {
		this.x = x;
		this.y = y;
		this.width = width;
		this.height = height;
	...
	/*
	 * Waits for the user to finish typing the message and then returns it when the
	 * user is finished
	 */
	...
	/*
	 * Once the user presses enter, this becomes true, as they are finished passing
	 * input
	 */
	public boolean isFinished() {
		// Refreshes window first to update all fields
		comp.repaint();
	...
	public void keyPressed(KeyEvent e) {
		// Manually repaints window whenever a key is pressed
		comp.repaint();
		...
		if (!entered && selected) {
			...
			if (keyCode == '\\') {
				message = "192.168.1.150"; // My LAN IP at home
			}
		}
